...
describe('PasswordRules', () => {
  ...
  it('should show all rules as not met for empty password', () => {
    ...
    listItems.forEach(item => {
      expect(item).toHaveClass('text-slate-400');
  ...
  it('should show min length rule as met for 8+ chars', () => {
    ...
    expect(minLengthItem).toHaveClass('text-green-400');
    expect(minLengthItem.querySelector('[data-testid="check-icon"]')).toBeInTheDocument();
    expect(minLengthItem.querySelector('[data-testid="x-icon"]')).not.toBeInTheDocument();
  });
  ...
  it('should show letter rule as met when password contains letter', () => {
    ...
    expect(letterItem).toHaveClass('text-green-400');
    expect(letterItem.querySelector('[data-testid="check-icon"]')).toBeInTheDocument();
    expect(letterItem.querySelector('[data-testid="x-icon"]')).not.toBeInTheDocument();
  });
  ...
  it('should show number rule as met when password contains number', () => {
    ...
    expect(numberItem).toHaveClass('text-green-400');
    expect(numberItem.querySelector('[data-testid="check-icon"]')).toBeInTheDocument();
    expect(numberItem.querySelector('[data-testid="x-icon"]')).not.toBeInTheDocument();
  });
  ...
  it('should handle partial rule satisfaction', () => {
    ...
    expect(listItems[0]).toHaveClass('text-slate-400');
    expect(listItems[0].querySelector('[data-testid="x-icon"]')).toBeInTheDocument();

    // Letter: met
    expect(listItems[1]).toHaveClass('text-green-400');
    expect(listItems[1].querySelector('[data-testid="check-icon"]')).toBeInTheDocument();

    // Number: met
    expect(listItems[2]).toHaveClass('text-green-400');
    expect(listItems[2].querySelector('[data-testid="check-icon"]')).toBeInTheDocument();
  });
});
