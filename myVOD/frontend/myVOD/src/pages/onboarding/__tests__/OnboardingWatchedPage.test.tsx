...
vi.mock("@/components/onboarding/MovieSearchCombobox", () => ({
  MovieSearchCombobox: ({
    ...
    onSelect: (option: { tconst: string; primaryTitle: string }) => void;
    disabled: boolean;
    ...
    return (
      <div
        data-testid="movie-search-combobox-wrapper"
        data-value={value}
      ...
      >
        ...
        <button
          ...
          onClick={() =>
            onSelect({ tconst: "tt0111161", primaryTitle: "Test Movie" })
          }
...
vi.mock("@/components/onboarding/SelectedMoviesList", () => ({
  SelectedMoviesList: ({
    ...
    <div
      data-testid="selected-movies-list"
...
const mockUseOnboardingWatchedController = vi.mocked(
...
describe("OnboardingWatchedPage", () => {
  it("should render all sections", () => {
    ...
    // Check layout title
    expect(screen.getByTestId("onboarding-layout")).toHaveAttribute(
      "data-title",
      "Oznacz filmy które już widziałeś"
    );
    ...
    expect(
      screen.getByText("Oznacz przynajmniej 3 filmy które już widziałeś")
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it("should keep search enabled when 3 movies selected", () => {
    ...
    const searchComboboxWrapper = screen.getByTestId(
      "movie-search-combobox-wrapper"
    );
  ...
  it("should show validation error when trying to finish with less than 3 movies", () => {
    ...
    expect(
      screen.getByText(
        "Oznacz przynajmniej 3 filmy jako obejrzane, aby zakończyć onboarding."
      )
    ).toBeInTheDocument();
