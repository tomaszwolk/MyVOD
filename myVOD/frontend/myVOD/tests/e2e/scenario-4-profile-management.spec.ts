...
test.describe('Scenario 4: Profile Management and Account Deletion', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // User is already authenticated via storageState in playwright.config.ts
    ...
    // Note: User authentication is handled by storageState in playwright.config.ts
    // If authentication fails, the test will fail at the first navigation attempt
  });
  ...
  test('User can manage profile preferences and delete account', async ({ page }) => {
    ...
    await loginPage.verifyLoginError('Account not found or invalid credentials');
